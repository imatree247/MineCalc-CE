...
#define VD 15
//unused ^ ^ ^ must edit run dda loop manually
...
#define MAGMABLOCK (reg_block_num+10)
#define FIRE (reg_block_num+11)
#define SANDSTONE (reg_block_num+12)
#define CACTUS (reg_block_num+13)

#define TEXTURENUM 13


#define SNOWCARPET (reg_block_num+TEXTURENUM-FLOORNUM+1)


#define FLOORNUM 1
#define FLOORVOXELS (reg_block_num+TEXTURENUM-FLOORNUM)


static uint8_t floorcolorsconst[]={255};
uint8_t* floorcolors=floorcolorsconst-FLOORVOXELS;
...
static const uint8_t hardness[block_num]=      {0,  1,    2,    1,    0,      255,    5,    3,    4,      4,    1, 1,1,1,2, /*textures*/0,1, 2,1,1,1,1,1,1,2,2,255,2,1,/*mobs*/255,255,255,255,255,255,255};
...
void furnace_stuff(uint8_t* block)//uint8_t* block)//need block because of syntax for array
{
	...
	while(true){
		...
		for(int i=scroll+1; i<reg_block_num+item_num+TEXTURENUM; ++i)//looping through inventory (inventory is an int[block_num+item_num])
		{
			if(c>35)
				break;
			c++;
...
static void (*block_is_usable[])(uint8_t*) ={use_craftingtable,  furnace_stuff, NULL, use_bed,NULL,   use_doorto,use_doorbo,use_doortc,use_doorbc,NULL,NULL,     NULL,NULL,NULL,NULL,NULL,hurtsheep,hurtsheep,hurtsheep,hurtsheep,hurtzombie,hurtzombie,hurtzombie};
...
#define DDA_ONCE(stepx,stepy,stepz,ddacountval)\
		if (tmaxx<tmaxy){\
			if (tmaxx<tmaxz){/*tmaxx is smallest*/\
				curr_block+=stepx;\
...
#define RUN_DDA_LOOP(stepx,stepy,stepz,quadrant)\
	{\
		DDA_ONCE(stepx,stepy,stepz,1)\
		dda1##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,2)\
		dda2##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,3)\
		dda3##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,4)\
		dda4##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,5)\
		dda5##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,6)\
		dda6##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,7)\
		dda7##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,8)\
		dda8##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,9)\
		dda9##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,10)\
		dda10##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,11)\
		dda11##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,12)\
		dda12##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,13)\
		dda13##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,14)\
		dda14##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,15)\
		dda15##q##quadrant:\
		DDA_ONCE(stepx,stepy,stepz,16)\
	}

// Generates all 15 steps for a single given quadrant
#define STEP_LIST(quad) \
    &&dda1##q##quad,  &&dda2##q##quad,  &&dda3##q##quad,  &&dda4##q##quad,  &&dda5##q##quad,  \
    &&dda6##q##quad,  &&dda7##q##quad,  &&dda8##q##quad,  &&dda9##q##quad,  &&dda10##q##quad, \
    &&dda11##q##quad, &&dda12##q##quad, &&dda13##q##quad, &&dda14##q##quad, &&dda15##q##quad

// Automatically populates all 8 quadrants in a flat array (120 elements total)
#define ALL_QUADRANTS \
    STEP_LIST(0), STEP_LIST(1), STEP_LIST(2), STEP_LIST(3), \
    STEP_LIST(4), STEP_LIST(5), STEP_LIST(6), STEP_LIST(7)


#define TCOLORSWITCH(quad_var, dda_count) \
    goto *dda_jump_table[((quad_var) << 4) - (quad_var) + (dda_count)]
...
void raycast_screen()
{
	void* dda_jump_table[] = { ALL_QUADRANTS };
	...
	*/
	...
	for (unsigned int y=0; y<184;y+=res){
		...
		for(unsigned int x=0; x<320;x+=res){
			...
			switch(quadrant) 
			{
				case 0: RUN_DDA_LOOP(-XJ, -YJ, -1,0); break; // All negative
				case 1: RUN_DDA_LOOP( XJ, -YJ, -1,1); break; // X positive
				case 2: RUN_DDA_LOOP(-XJ,  YJ, -1,2); break; // Y positive
				case 3: RUN_DDA_LOOP(XJ,  YJ, -1,3); break; // X, Y positive
				
				case 4: RUN_DDA_LOOP( -XJ, -YJ,  1,4); break; // Z positive
				case 5: RUN_DDA_LOOP(  XJ, -YJ,  1,5); break; // X, Z positive
				case 6: RUN_DDA_LOOP( -XJ,  YJ,  1,6); break; // Y, Z positive
				case 7: RUN_DDA_LOOP(  XJ,  YJ,  1,7); break; // All positive
			}
			*tempval+=res;
			continue;
			blocky_stuff:
			...
			else //textured blocks. slower. 
			{
				...
				if(id>FLOORVOXELS)//like carpets or redstone or pressure plates and stuff
				{
					if(tmaxy<tmaxx)
					{
						if(tmaxy<tmaxz)
				...
				switch ((uint8_t)d) {
					case 0://x
					{
						...
						{
							tmaxx+=deltax;					
							TCOLORSWITCH(quadrant,ddacount);
						}
						break;
					}
...
inline void draw_item(int temp,int x,int y,int show_num)
{
	...
	{
		Sprite_8x8to32x32(&texturespritebuffer[(temp-(reg_block_num+1))<<6],&gfx_vbuffer[y][x]);
	}
	else//if item
	{
...
int main(void){
	...
	*/
	...
	uint8_t* hotbar =&ghotbar[0];
	{
		...
		for(int i=reg_block_num+1; i<invenblocknum+1;++i)   // textured blocks: cache at 8x8
		{
			gfx_FillScreen(0);
			draw_itemnobuffer(i,0,0);
			int c=(i-(reg_block_num+1))<<6;
			...
			if (kb_IsDown(kb_Key2nd)) {
			 ...
			 if ((kb_Data[3] & kb_GraphVar)||is_in_crafting_table) {//xton key. inventory/crafting. haven't implemented it yet tho.
				...
				while (!(kb_Data[3] & kb_GraphVar||kb_IsDown(kb_KeyClear)))// if press it again, break
				{
					...
					int count=0;
					for (int i=scroll+1;i<reg_block_num+item_num+TEXTURENUM;++i){//drawing inventory. 0 is air.
